...
const PatientHistory = () => {
  ...
  const sampleMedicalHistory = [
    {
      ...
      treatment: 'Prescribed vitamin D supplements, recommended lifestyle changes',
      medications: [
      ...
      labReports: [
        { id: 'LAB-001', name: 'Complete Blood Count', status: 'completed', uploaded: '2024-03-16' },
        { id: 'LAB-002', name: 'Vitamin D Test', status: 'completed', uploaded: '2024-03-16' }
      ],
      imagingReports: [],
      followUp: '2024-04-15',
      notes: 'Patient shows improvement in overall health. Continue with current medication.'
    },
    {
      id: 2,
      date: '2024-02-28',
      ...
      medications: [
        { name: 'Alprazolam', dosage: '0.5mg', frequency: 'As needed', duration: '1 week' }
      ],
      vitalSigns: {
      ...
      labReports: [
        { id: 'LAB-003', name: 'Troponin Test', status: 'completed', uploaded: '2024-02-28' },
        { id: 'LAB-004', name: 'ECG', status: 'completed', uploaded: '2024-02-28' }
      ],
      imagingReports: [
        { id: 'IMG-001', name: 'Chest X-Ray', status: 'completed', uploaded: '2024-02-28' }
      ],
      followUp: '2024-03-07',
      notes: 'Patient responded well to treatment. No cardiac abnormalities found.'
    },
    {
      id: 3,
      date: '2024-01-20',
      ...
      labReports: [
        { id: 'LAB-005', name: 'HbA1c Test', status: 'completed', uploaded: '2024-01-21' },
        { id: 'LAB-006', name: 'Blood Glucose', status: 'completed', uploaded: '2024-01-21' }
      ],
      imagingReports: [],
      followUp: '2024-02-20',
      notes: 'Blood sugar levels are well controlled. Continue current treatment plan.'
    },
    {
      id: 4,
      date: '2023-12-10',
      ...
      labReports: [],
      imagingReports: [
        { id: 'IMG-002', name: 'Dental X-Ray', status: 'completed', uploaded: '2023-12-10' }
      ],
      followUp: '2024-06-10',
    ...
    },
    {
      ...
      labReports: [
        { id: 'LAB-007', name: 'Comprehensive Metabolic Panel', status: 'completed', uploaded: '2023-11-06' },
        { id: 'LAB-008', name: 'Lipid Panel', status: 'completed', uploaded: '2023-11-06' },
        { id: 'LAB-009', name: 'Thyroid Function Test', status: 'completed', uploaded: '2023-11-06' }
      ],
      imagingReports: [],
      followUp: '2024-11-05',
      notes: 'Patient is in good health. Continue with preventive care measures.'
    }
  ];
  ...
  const years = [...new Set(medicalHistory.map(item => new Date(item.date).getFullYear().toString()))].sort((a, b) => b - a);
    ...
    return (
      <div className="flex items-center justify-center py-12">
        <div className="text-center">
          ...
          <p className="text-gray-600">The patient with ID "{patientId}" could not be found.</p>
        </div>
      </div>
    );
  }
      ...
      <div className="bg-white rounded-2xl p-6 shadow-sm border border-[#E9DFC3]/50">
        <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4">
          <div>
            ...
            <p className="text-gray-600">Complete timeline of medical events, treatments, and reports</p>
          </div>
        ...
        <div className="p-6">
          {filteredHistory.length === 0 ? (
            <div className="text-center py-12">
              ...
              <p className="text-gray-600">No medical events match the selected filters.</p>
            </div>
          ) : (
            <motion.div
            ...
            >
              {/* Timeline line */}
              <div className="absolute left-8 top-0 bottom-0 w-0.5 bg-[#E9DFC3]"></div>
              ...
              {filteredHistory.map((event, index) => (
                ...
                >
                  {/* Timeline dot */}
                  <div className="absolute left-6 w-4 h-4 bg-[#0118D8] rounded-full border-4 border-white shadow-lg transform -translate-x-1/2"></div>

                  {/* Event card */}
                  <div className="ml-16 bg-gray-50 rounded-2xl p-6 border border-[#E9DFC3]/50 hover:border-[#1B56FD]/20 transition-all duration-300">
                    {/* Event header */}
                    <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-4">
                      <div className="flex items-center gap-3">
                        <div className={`w-10 h-10 rounded-xl flex items-center justify-center ${getTypeColor(event.type)}`}>
                          {React.createElement(getCategoryIcon(event.category), { className: 'w-5 h-5' })}
                        </div>
                        <div>
                    ...
                    <div className="flex items-center gap-2 mb-4 text-sm text-gray-600">
                      {React.createElement(getDepartmentIcon(event.department), { className: 'w-4 h-4' })}
                      ...
                      <div className="mb-4">
                        ...
                        <div className="space-y-2">
                          {event.medications.map((med, idx) => (
                            <div key={idx} className="bg-white rounded-lg p-3 border border-[#E9DFC3]/50">
                    ...
                    {event.vitalSigns && (
                      <div className="mb-4">
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
                          {Object.entries(event.vitalSigns).map(([key, value]) => (
                            <div key={key} className="bg-white rounded-lg p-3 border border-[#E9DFC3]/50 text-center">
                    ...
                    {/* Reports */}
                    {(event.labReports.length > 0 || event.imagingReports.length > 0) && (
                      <div className="mb-4">
                        <h4 className="text-sm font-medium text-gray-700 mb-2">Reports</h4>
                        <div className="space-y-2">
                          {event.labReports.map((report) => (
                            <div key={report.id} className="flex items-center justify-between bg-white rounded-lg p-3 border border-[#E9DFC3]/50">
                              <div className="flex items-center gap-2">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      {event.followUp && (
                        <div>
                          ...
                          <p className="text-gray-600 text-sm">{new Date(event.followUp).toLocaleDateString()}</p>
                        </div>
                      )}
                      {event.notes && (
                        <div>
